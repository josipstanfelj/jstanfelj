...
public class EditDonationController {
        ...
        if(showConfirmationDialog("Are you sure you want make this changes?")){
            ...
            if(projectNameCheckBox.isSelected() && amountCheckBox.isSelected()){
                ...
                if (isDouble(newAmountString) && optionalProject.isPresent()) {
                    ...
                    Thread thread2 = new Thread(updateAmount);
                    thread2.start();
                    waitThread(thread2);
                ...
                if (projectNameCheckBox.isSelected()) {
                    String newProjectName = newProjectNameTextField.getText();
                    Optional<Project> optionalProject = Database.getProjectByName(newProjectName);
                    if (optionalProject.isPresent()) {
                        UpdateProjectCurrentAmountAfterDeletingDonationThread editAmount = new UpdateProjectCurrentAmountAfterDeletingDonationThread(donation);
                        Thread thread = new Thread(editAmount);
                        thread.start();
                        waitThread(thread);
                        UpdateDonationFundingProjectThread updateDonationFundingProjectThread = new UpdateDonationFundingProjectThread(donation.getDonationId(), optionalProject.get().getProjectId());
                        Thread thread1 = new Thread(updateDonationFundingProjectThread);
                        thread1.start();
                        waitThread(thread1);
                        ...
                        Thread thread2 = new Thread(addDonationToProjectThread);
                        thread2.start();
                        waitThread(thread2);
                        noteTheChange("Editing a donation funded project.",oldProject.getProjectName(),newProjectName);
                    } else {
                    ...
                    thread.start();
                    waitThread(thread);
                    UpdateProjectCurrentAmountAfterEditingDonationThread updateAmount = new UpdateProjectCurrentAmountAfterEditingDonationThread(donation,newAmount);
                    Thread thread1 = new Thread(updateAmount);
                    thread1.start();
                    waitThread(thread1);
                    noteTheChange("Editing a donation amount.",String.valueOf(donation.getDonationAmount()),newAmountString);
                }
    ...
    public void noteTheChange(String typeOfChange,String before,String after){
        NoteTheChange<String> change = new NoteTheChange<>("Donor",typeOfChange,before,after);
